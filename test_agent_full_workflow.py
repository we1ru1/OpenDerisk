...
async def test_local_sandbox():
    ...
    try:
        ...
        # 写入文件
        await sandbox.file.create(test_file, test_content)
        logger.info(f"✅ 文件写入成功: {test_file}")

        # 读取文件
        file_info = await sandbox.file.read(test_file)
        assert file_info.content == test_content, "文件内容不匹配"
        ...
        test_results.add_test("Local 沙箱功能", False, error=str(e))
...
async def test_file_system():
    ...
    try:
        ...
        from derisk.agent.core.memory.gpts import GptsMemory, FileType
    ...
    try:
        ...
        for vis_type in vis_types:
            try:
                ...
                logger.warning(f"⚠️ Vis 组件不可用: {vis_type} - {e}")
        ...
        test_results.add_test(
            "Vis 渲染推送功能", True, details={"vis_components": results}
        )
        ...
        traceback.print_exc()
        test_results.add_test("Vis 渲染推送功能", False, error=str(e))
...
async def test_sandbox_manager():
    ...
    try:
        ...
        test_results.add_test(
            "沙箱管理器",
            True,
            details={"initialized": False, "has_client": manager.client is None},
        )
        ...
        traceback.print_exc()
        test_results.add_test("沙箱管理器", False, error=str(e))


async def main():
    """主测试函数"""
    logger.info("\n" + "=" * 80)
    logger.info("开始 Agent 全对话流程验证测试")
    logger.info("=" * 80)
