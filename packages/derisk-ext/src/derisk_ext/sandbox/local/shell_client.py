    ...
    async def exec_command(
        ...
        *,
        ...
        if not target_cwd:
            ...
            # 但是本地沙箱比较特殊，如果用户传了 /tmp/foo，是指宿主机 /tmp/foo 还是沙箱内的 /tmp/foo？
            # 既然是 LocalSandbox，我们最好还是限制在沙箱根目录下
            # 这里的实现与 LocalFileClient._get_physical_path 类似
        ...
        try:
            ...
            try:
                ...
                status_val = "completed" if exit_code == 0 else "failed" # 注意 BashCommandStatus 类型
