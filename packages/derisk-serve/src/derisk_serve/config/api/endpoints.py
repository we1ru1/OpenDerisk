...
async def force_update(key: str, operator: str, service: Service = Depends(get_service)) -> Result[ServerResponse]:
    try:
        await service.force_update(key, operator)
        return Result.succ(data=None)
...
async def query(category: str, service: Service = Depends(get_service)) -> Result[ServerResponse]:
    """Query Config entities by category
    ...
    Returns:
        ServerResponse: The response
    """
    return Result.succ(service.get_by_category(category))
