...
class ServeEntity(Model):
    ...
    resources = Column(Text(length=2 ** 31 - 1), nullable=True, comment="应用当前版本的Resources信息")
    details = Column(String(2000), nullable=True, comment="应用当前版本的小弟details信息")
    recommend_questions = Column(Text, nullable=True, comment="当前版本配置设定的推进问题信息")
    ...
    is_published = Column(SmallInteger, nullable=True, default=0, comment="当前版本配置的备注描述")
    gmt_last_edit = Column(DateTime, nullable=True, comment="当前版本配置最后一次内容编辑时间")
    editor = Column(String(255), nullable=True, comment="当前版本配置最后修改者")
    ext_config = Column(Text(length=2 ** 31 - 1), nullable=True, comment="当前版本配置的扩展配置，各自动态扩展的内容")
    system_prompt_template = Column(Text, nullable=True, comment="当前版本配置的system prompt模版")
    user_prompt_template = Column(Text, nullable=True, comment="当前版本配置的user prompt模版")
...
def _load_team_context(
    ...
    """
    def _str_to_team_context(cls_type: Type[Union[TeamContext, BaseModel]], content: Optional[str] = None)-> Optional[TeamContext]:
        try:
            agent_ctx = None
            if team_context:
                ...
                elif isinstance(team_context, dict) and cls_type.model_validate(team_context):
...
class ServeDao(BaseDao[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def to_update_dict(self, request: ServeRequest):
        ...
        return {
            ...
            "team_context": json.dumps(request.team_context.to_dict(),
                                       ensure_ascii=False) if request.team_context else None,
            ...
            "layout": json.dumps(request.layout.to_dict(), ensure_ascii=False) if request.layout else None,
            "custom_variables": _bm_to_str(request.custom_variables),
            "llm_config": json.dumps(request.llm_config.to_dict(),
                                     ensure_ascii=False) if request.llm_config else None,
    ...
    def to_request(self, entity: ServeEntity) -> ServeRequest:
        ...
        gmt_created_str = entity.gmt_create.strftime("%Y-%m-%d %H:%M:%S") if entity.gmt_create else None
        gmt_modified_str = entity.gmt_modified.strftime("%Y-%m-%d %H:%M:%S") if entity.gmt_modified else None
        return ServeRequest(
            id=entity.id,
            code=entity.code,
            ...
            ext_config=json.loads(entity.ext_config),
