...
if TYPE_CHECKING:
    ...
    from derisk.datasource.manages import ConnectorManager
...
class Config(metaclass=Singleton):
    ...
    def __init__(self) -> None:
        ...
        from derisk.core._private.prompt_registry import PromptTemplateRegistry
        ...
        self.LOCAL_DB_NAME = os.getenv("LOCAL_DB_NAME", "derisk")
        ...
        # When we use the rest API provided by deployment frameworks like fastchat as
        # a proxyllm, "PROXYLLM_BACKEND" is the model they actually deploy.
        # We need to use "PROXYLLM_BACKEND" to load the prompt of the corresponding
        # scene.
        ...
        # Log level
        self.DERISK_LOG_LEVEL = os.getenv("DERISK_LOG_LEVEL", "INFO")
        ...
        # global derisk api key
        self.API_KEYS = os.getenv("API_KEYS", None)
        self.ENCRYPT_KEY = os.getenv("ENCRYPT_KEY", "your_secret_key")

        # Non-streaming scene retries
        self.DERISK_APP_SCENE_NON_STREAMING_RETRIES_BASE = int(
            os.getenv("DERISK_APP_SCENE_NON_STREAMING_RETRIES_BASE", 1)
        )
        # Non-streaming scene parallelism
        self.DERISK_APP_SCENE_NON_STREAMING_PARALLELISM_BASE = int(
            os.getenv("DERISK_APP_SCENE_NON_STREAMING_PARALLELISM_BASE", 1)
        )
