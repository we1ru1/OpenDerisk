...
class AsyncRestClient:
    def __init__(
        ...
        **kwargs
        ...
        if not _default_ssl_context:
            # 创建不验证 SSL 的上下文
            ssl_context = ssl.create_default_context()
            ssl_context.check_hostname = False
            ssl_context.verify_mode = ssl.CERT_NONE
            _default_ssl_context = ssl_context

        self.base_url = base_url.rstrip("/") if base_url else None
        self.connector = aiohttp.TCPConnector(limit=max_connections, ssl=_default_ssl_context)
    ...
    async def stream(self, method: str, endpoint: str, **kwargs):
        """
        流式请求方法
        ...
        for attempt in range(self.retries + 1):
            try:
                logger.info(f"stream url:{url},headers:{headers},{kwargs}")
                async with self.session.request(
                    method=method, url=url, headers=headers, **kwargs
                ) as response:
                    response.raise_for_status()
