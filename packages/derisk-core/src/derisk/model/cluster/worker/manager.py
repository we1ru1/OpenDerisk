...
from derisk.core import ModelMetadata, ModelOutput
from derisk.core.interface.parameter import (
    BaseDeployModelParameters,
    EmbeddingDeployModelParameters,
    LLMDeployModelParameters,
    RerankerDeployModelParameters,
)
from derisk.model.base import ModelInstance, WorkerApplyOutput, WorkerSupportedModel
...
from derisk.model.cluster.manager_base import (
    WorkerManager,
    WorkerManagerFactory,
    WorkerRunData,
)
from derisk.model.cluster.registry import ModelRegistry
from derisk.model.cluster.storage import ModelStorage, ModelStorageItem
...
from derisk.model.utils.llm_utils import list_supported_models
from derisk.util.executor_utils import run_async_tasks
from derisk.util.fastapi import create_app, register_event_handler
    ...
    async def _start_all_worker(
        ...
        async def _start_worker(worker_run_data: WorkerRunData):
            ...
            try:
                ...
                logger.exception(f"{info} start failed!")
...
def _setup_fastapi(
    ...
    if worker_params.standalone:
        from derisk.model.cluster.controller.controller import initialize_controller
...
def _create_local_model_manager(
    ...
    from derisk.util.net_utils import _get_ip_address
    ...
    if not worker_params.register or not worker_params.controller_addr:
        ...
        from derisk.model.cluster.controller.controller import ModelRegistryClient
...
def _build_worker(
    ...
    if worker_class:
        ...
        if worker_type is None or worker_type == WorkerType.LLM:
            ...
            from derisk.model.cluster.worker.embedding_worker import (
...
def run_worker_manager(
    ...
    **kwargs,
    ...
    trace_file = trace_config.file or os.path.join(
        "logs", "derisk_model_worker_manager_tracer.jsonl"
    )
    initialize_tracer(
        trace_file,
        system_app=system_app,
        root_operation_name=trace_config.root_operation_name or "DERISK-ModelWorker",
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="DERISK Model Worker")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
