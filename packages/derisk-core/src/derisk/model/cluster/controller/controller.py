...
from derisk.model.base import ModelInstance
from derisk.model.cluster.registry import EmbeddedModelRegistry, ModelRegistry
from derisk.model.parameter import DBModelRegistryParameters, ModelControllerParameters
from derisk.util.api_utils import APIMixin
from derisk.util.api_utils import _api_remote as api_remote
from derisk.util.api_utils import _sync_api_remote as sync_api_remote
...
def initialize_controller(
    ...
    if app:
        ...
        trace_file = trace_config.file or os.path.join(
            "logs", "derisk_model_controller_tracer.jsonl"
        )
        initialize_tracer(
            trace_file,
            system_app=system_app,
            root_operation_name=trace_config.root_operation_name
            or "DERISK-ModelController",
...
async def api_register_instance(request: ModelInstance):
    with root_tracer.start_span(
        "derisk.model.controller.register_instance", metadata=request.to_dict()
    ):
...
async def api_deregister_instance(
    ...
    with root_tracer.start_span(
        "derisk.model.controller.deregister_instance", metadata=instance.to_dict()
    ):
...
def _create_registry(controller_params: ModelControllerParameters) -> ModelRegistry:
    ...
    if not controller_params.registry:
        ...
        from derisk.datasource.rdbms.base import (
            RDBMSConnector,
            RDBMSDatasourceParameters,
        )
        from derisk.model.cluster.registry_impl.storage import StorageModelRegistry
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="DERISK API Server")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
...
def run_model_controller(config_file: str):
    from derisk.configs.model_config import LOGDIR, ROOT_PATH
