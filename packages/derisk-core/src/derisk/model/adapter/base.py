...
from derisk.core.interface.media import MediaProcessor
...
from derisk.model.base import ModelType, SupportedModel
...
class EmbeddingModelAdapter(ABC):
    """New Adapter for DERISK Embedding models"""
...
class LLMModelAdapter(ABC):
    """New Adapter for DERISK LLM models"""
    ...
    def _set_conv_converted_messages(
        ...
        if system_messages:
            if len(system_messages) > 1:
                # Compatible with derisk complex scenarios, the last system will protect
    ...
    def model_adaptation(
        ...
        # Some model context to derisk server
        model_context = {
