    ...
    const ImageRender = ({ item, handleDelete, index }: any) => {
      ...
      if (previewUrl) {
        return (
          <div
            key={`img-${index}`}
            className='relative flex flex-col border border-[#e3e4e6] dark:border-[rgba(255,255,255,0.6)] rounded-lg p-2'
          >
            {/* Delete icon */}
            <span
              className='absolute top-[-2px] right-[1] cursor-pointer z-10 text-gray-500'
      ...
      <div className='group/item flex flex-wrap gap-2 mt-2'>
        {resources.map((item, index) => {
          ...
          if (item.type === 'image_url' && item.image_url?.url) {
            return ImageRender({item, handleDelete, index});
            ...
            return (
              <div
                key={`file-${index}`}
                className='relative flex items-center justify-between border border-[#e3e4e6] dark:border-[rgba(255,255,255,0.6)] rounded-lg py-2 px-3'
              >
                {/* Delete icon */}
                <span
                  className='absolute top-[-2px] right-[1] cursor-pointer z-10 text-gray-500'
